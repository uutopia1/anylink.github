...
public class AH_54_BAT_Logging extends DefaultAdapterMessageHandler {

	private static final Logger logger = Logger.getLogger(AH_54_BAT_Logging.class.getName());
		...
		if(logger.isLoggable(Level.SEVERE)) {
			try {
				logger.severe("+++ " + sysId + " (GUID:" + getConnectionGUID() + ")\n"
						+ "[RECV DATA][F"+OPR_TRXCD+"][BAT][LEN:" + String.format("%04d", bMessage.length) + "]:[" + new String(bMessage, LOCALE) + "]\n");
	...
	public Object send(Object message) throws AnyLinkException {
		...
		if(logger.isLoggable(Level.SEVERE)) {
			try {
				logger.severe("+++ " + sysId + " (GUID:" + getConnectionGUID() + ")\n"
						+ "[SEND DATA][F"+OPR_TRXCD+"][BAT][LEN:" + String.format("%04d", bMessage.length) + "]:[" + new String(bMessage, LOCALE) + "]\n");
			} catch (UnsupportedEncodingException e) {}
		}
